...
<section class="space-y-6">
  {#each projects as project, i}
    <Card.Root class="py-4 overflow-hidden border-2 transition-all hover:border-primary/50">

      <div class="aspect-video w-full overflow-hidden bg-muted/50 flex items-center justify-center px-4 py-0 rounded-md">
        {#if project.image}
          <a href={project.github} target="_blank" class="border-4 border-(--blendedMagenta)/30 rounded-md">
          <img
            src={project.image} 
            alt={project.title}
            class="h-full w-full object-contain transition-transform duration-300 hover:scale-105 rounded-md"
          />
          </a>
        {:else}
      ...
      <Card.Header class="pb-2">
        <div class="flex items-center justify-between">
          ...
          <div data-electric-cursor class="flex items-center justify-center">
            <Button variant="outline" size="icon" href={project.github} target="_blank" >
              <svg
